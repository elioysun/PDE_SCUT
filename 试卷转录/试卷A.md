...
> - [x] 主对应：`raw/作业/第03章/习题3-2.md` 1(4)、2(3)，练 Neumann-Neumann 边界和零模。
> - [x] 次对应：`raw/作业/第03章/习题3-2.md` 2(1)，练 Dirichlet-Dirichlet 边界的正弦本征函数。
...
>
> 核对说明：PDF 内嵌答案对混合边界使用 $\omega_n=\dfrac{(2n+1)\pi}{2l}$ 的编号口径，等价于这里从 $n=1$ 开始写成 $\dfrac{(2n-1)\pi}{2l}$。
...
> [!success]- 参考答案
> PDF 内嵌答案给出的三维 Laplace 方程基本解为
>
> $$
> U_0=\frac1r,\qquad r\ne0.
> $$
>
> 其中 $r=|M-M_0|$；极点处为奇异点。有些 Green 函数归一化会把三维基本解写成 $\dfrac{1}{4\pi r}$，但按本 PDF 原答案填空应优先写 $1/r$。
>
...
>
> $$
> y=A J_{\sqrt5}(x)+B J_{-\sqrt5}(x).
> $$
>
> 因为 $\sqrt5$ 不是整数，也可等价写成
>
> $$
...
> $$
>
> 核对说明：PDF 参考答案使用 $s=\frac12(y^2-x^2)$、$t=\frac12(y^2+x^2)$，与本文的 $\xi,\eta$ 只差线性缩放和换号；本文标准形与其等价。PDF 末行常数项疑似漏分母，本文按链式法则推导后的式子保留。
